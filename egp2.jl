...
for n in range(Nt):
    ...
    for i in range(1, Nx - 1):
        D = D_profile[i]
        alpha = D * dt / dx**2
        C_new[i] = C[i] + alpha * (C[i+1] - 2*C[i] + C[i-1]) - k_deg * C[i] * dt

    # Absorption at the end boundary (Michaelis-Menten)
    C_last = C[-1]
    absorption_rate = Vmax * C_last / (Km + C_last + 1e-12)  # prevent zero division
    absorbed_amount = absorption_rate * dt  # mol/m²

    # Approximate loss at the boundary
    C_new[-1] = C[-2] - absorption_rate * dx * dt
    if C_new[-1] < 0:
        C_new[-1] = 0

    # No-flux boundary at the entry
    C_new[0] = C_new[1]
