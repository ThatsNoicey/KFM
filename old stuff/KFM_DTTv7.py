    ...
    def _angle_callback(self, sender, data):
        ...
        try:
            ...
            with self.lock:
                ...
                # Always update last_angle for real-time display
                self.last_angle = angle
    
                # Session detection logic:
                # Start a session if the angle is above the flex threshold.
                if angle > KFMS_PARAMS['flex_threshold']:
                    if not self.current_session['session_active']:
                        self._start_new_session()
                    self._update_session(angle)
                # Finalize the session if it is active and angle drops below threshold.
...
def run_flask(analyzer):
    """Web server configuration with error handling"""
    app = Flask(__name__)
    ...
    def get_current_data():
        ...
        try:
            with analyzer.lock:
                # Use the last_angle if no live angle is available
                if analyzer.current_session.get('live_angles'):
                    current_angle = analyzer.current_session['live_angles'][-1]
                else:
                    current_angle = analyzer.last_angle
                return jsonify({
    ...
    try:
        ...
        app.run(
            ...
            debug=False,
            use_reloader=False,
            threaded=True  # Important for async compatibility
        )
    except OSError as e:
...
#    patient_pos = input("Enter patient position Led down or Sat L/S").strip()
