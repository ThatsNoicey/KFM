...
class KFM_Analyzer:
    ...
    SESSION_STATES = Enum('SessionStates', 'INACTIVE STARTING ACTIVE STOPPING')
    
    def _start_new_session(self):
        if self.current_session['state'] != SESSION_STATES.INACTIVE:
            logging.warning("Session start requested while active")
            return
        ...
        # Delay state transition to prevent race conditions
        time.sleep(0.1)
        self.current_session['state'] = SESSION_STATES.ACTIVE
    ...
    async def _setup_notifications(self):
        ...
        found_uuids = {char.uuid for service in svcs for char in service.characteristics}
        
        required_uuids = set(BLE_CONFIG['uuids'].values())
        if not required_uuids.issubset(found_uuids):
            missing = required_uuids - found_uuids
            raise ValueError(f"Missing characteristics: {missing}")
        try:
