    ...
    def _finalize_session(self):
        with self.session_lock:
            ...
            try:
                ...
                logging.error(f"Session finalization failed: {e}")
                self._set_session_state(SessionState.INACTIVE)
    ...
    def _angle_callback(self, sender, data):
        try:
            ...
            with data_lock:
                ...
                self.current_session['live_angles'].append((angle, datetime.now()))
                ...
                if angle > KFMS_PARAMS['flex_threshold']:
                    ...
                    if self.session_is_active():
                        if self.current_session['low_angle_start'] is None:
                            self.current_session['low_angle_start'] = datetime.now()
                        else:
                            duration = (datetime.now() - self.current_session['low_angle_start']).total_seconds()
                            if duration >= 5:
            ...
            logging.error(f"Angle processing error: {e}")
